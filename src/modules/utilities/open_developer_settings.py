...
class OpenDeveloperSettings:
    ...
    def open_developer_settings(logger=None, log_file_path=None, app_translator=None):
        developer_settings_uri = "ms-settings:developers"

        def open_with_startfile():
            logger.info("Opening Developer Settings page via os.startfile.")
            os.startfile(developer_settings_uri)

        def open_with_cmd():
            logger.info("Opening Developer Settings page via CMD.")
        ...
        for method in methods:
            try:
                method()
                logger.info(f"Successfully opened the Developer Settings page via {method.__name__}.")
                return
            except Exception as e:
                last_error = e
                logger.warning(f"{method.__name__} Failed to Open the Developer Settings Page: {e}")
                continue
        logger.error("All methods failed to open the Developer Settings page.")
