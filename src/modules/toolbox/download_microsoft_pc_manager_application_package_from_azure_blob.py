...
class DownloadMicrosoftPCManagerApplicationPackageFromAzureBlob():
    ...
    def execute(self):
        ...
        try:
            ...
            self.logger.info(f"Opened Azure Blob Page via os.startfile: {download_url}")
            return True
        ...
        try:
            subprocess.run(
                ["cmd.exe", "/C", "start", "Azure Blob Page", f"{download_url}"],
                check=True,
                creationflags=subprocess.CREATE_NO_WINDOW
            )
            self.logger.info(f"Opened Azure Blob Page via CMD: {download_url}")
            return True
        ...
        try:
            ...
            self.logger.info(f"Opened Azure Blob Page via PowerShell: {download_url}")
            return True
        except Exception as e_windows_powershell:
            self.logger.error(f"Failed to open Azure Blob page via all methods. Error: {e_windows_powershell}")
            return False
