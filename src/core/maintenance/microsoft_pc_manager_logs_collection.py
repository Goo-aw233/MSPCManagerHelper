...
import os
import subprocess
from ...gui.modules import GetProgramResources
...
class MicrosoftPCManagerLogsCollection:
    ...
    def __init__(self):
        ...
        self._BASIC_LOGS_FOLDER = os.path.join(os.getenv("UserProfile"), "Desktop", "microsoft_pc_manager_logs_folder")
        if not os.path.exists(self._BASIC_LOGS_FOLDER):
    ...
    def _set_paths():
        ...
        _APPDATA_CLR_4_0_LOGS_SOURCE = os.path.join(os.getenv("LocalAppData"), "Microsoft", "CLR_v4.0", "UsageLogs")
        # EXE Installer Log Source Path
        _EXE_SETUP_LOGS_SOURCE = os.path.join(os.getenv("ProgramData"), "Windows Master Setup")
        # Log Collection Destination Path
        _LOGS_DESTINATION = os.path.join(os.getenv("UserProfile"), "Desktop", "microsoft_pc_manager_logs_folder", full_time_str)
        # Log Zip Archive Output Path
        _LOGS_ZIP_ARCHIVE_OUTPUT_PATH = os.path.join(os.getenv("UserProfile"), "Desktop")
        # Program Logs Source Path
        _PROGRAM_LOGS_SOURCE_PATH = os.path.join(os.getenv("ProgramData"), "Windows Master Store")
        # The Source Path of the CLR_v4.0 Log Under %SystemRoot%
        _SYSTEMROOT_CLR_4_0_LOGS_SOURCE = os.path.join(os.getenv("SystemRoot"), "System32", "config", "systemprofile", "AppData", "Local", "Microsoft", "CLR_v4.0", "UsageLogs")
